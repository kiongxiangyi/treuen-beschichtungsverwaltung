...
export default function WareneingangRueckgabe({ articleDB, rueckgabe }) {
  ...
  const handleChangeMenge = (value, index) => {
    ...
    if (index !== 0) {
      ...
      if (newValue >= totalQuantity) {
        // If the new value exceeds total quantity, adjust both the specified item and the first item
        newMengeSteckbretter[index] = totalQuantity;
        // Set all other quantities to 0 except the specified item
      ...
      const diff = newValue - mengeSteckbretter[0]; // Calculate the difference in quantity
      // If index is 0, handle the adjustment of the first item's quantity
      if (newValue >= totalQuantity) {
        // If the new value exceeds total quantity, set the first item's quantity to total quantity
        newMengeSteckbretter[0] = totalQuantity;
        // Set all other quantities to 0 except the first item
        for (let i = 1; i < anzahlSteckbretter; i++) {
          newMengeSteckbretter[i] = 0;
        }
      } else {
        // If the new value is less than or equal to total quantity, update the first item's quantity
        newMengeSteckbretter[0] = newValue;

        // Adjust the last newMengeSteckbretter by diff
        let remainingDiff =
          newValue - mengeSteckbretter[mengeSteckbretter.length - 1];
        for (let i = mengeSteckbretter.length - 1; i >= 0; i--) {
          if (remainingDiff >= 0) {
            // Subtract remaining difference from the current item
            const diffToSubtract = Math.min(
              remainingDiff,
              newMengeSteckbretter[i]
            );
            newMengeSteckbretter[i] -= diffToSubtract;
            remainingDiff -= diffToSubtract;
          } else {
            // If remainingDiff is negative, break the loop
            break;
          }
        }
      }
  ...
  const [activeIndex, setActiveIndex] = useState(null); // To keep track of the active input field index

  const toggleNumberPad = (index) => {
    setNumberPadVisible(!numberPadVisible);
    setActiveIndex(index);
  };

  const handleConfirm = (value) => {
    const newMengeSteckbretter = [...mengeSteckbretter];
    newMengeSteckbretter[activeIndex] = value;
      ...
      inputs.push(
        <tr key={`steckbretter-${i}`}>
          ...
          <th className="tabledata">
            ...
            <button onClick={() => toggleNumberPad(i)}>...</button>
            <CustomNumberPad
              visible={numberPadVisible && activeIndex === i}
              onConfirm={handleConfirm}
              onHide={() => setNumberPadVisible(false)}
            />
          </th>
        </tr>
      );
